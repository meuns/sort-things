...
void merge_sort_hybrid(int* keys, const int key_count, int* temp_keys)
{
  ...
  for (int left_key_count = 16; left_key_count <= key_count; left_key_count <<= 1)
  {
    ...
    {
      merge_keys(&input_keys[merge_index], left_key_count, &input_keys[merge_index + left_key_count], right_key_count, &output_keys[merge_index], merge_compare_default, merge_copy_default);
